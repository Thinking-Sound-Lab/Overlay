...
export class WindowManager {
  ...
  createRecordingWindow(): BrowserWindow {
    ...
    const currentDisplayId =
      this.displayManagerService!.getCurrentRecordingDisplayId()!;
    const position =
      this.displayManagerService!.calculateRecordingWindowPosition(
        currentDisplayId
      )!;
  ...
  createInformationWindow(): BrowserWindow {
    ...
    const position =
      this.displayManagerService!.calculateInformationWindowPosition(
        currentDisplayId!,
  ...
  getRecordingWindow(): BrowserWindow | null {
    return this.recordingWindow && !this.recordingWindow.isDestroyed()
      ? this.recordingWindow
      : null;
  }

  getInformationWindow(): BrowserWindow | null {
    return this.informationWindow && !this.informationWindow.isDestroyed()
  ...
  showRecordingWindow(): void {
    ...
    if (window && !window.isVisible()) {
      console.log(
        "[WindowManager] Showing recording window (onboarding completed)"
      );
      window.show();
    }
  }
  ...
  hideRecordingWindow(): void {
    ...
    if (window && window.isVisible()) {
      console.log(
        "[WindowManager] Hiding recording window (onboarding not completed)"
      );
      window.hide();
    }
  }
  ...
  initializeDisplayTracking(
    ...
    this.displayManagerService.on(
      "move-recording-window",
      (event: MoveRecordingWindowEvent) => {
        this.handleRecordingWindowMove(event);
      }
    );

    this.displayManagerService.on(
      "recording-window-position-update-needed",
  ...
  private handleRecordingWindowMove(event: MoveRecordingWindowEvent): void {
    ...
    console.log(
      `[WindowManager] *** RECEIVED MOVE EVENT *** for display ${displayId}`
    );
    console.log(`[WindowManager] Event details:`, event);
    console.log(
      `[WindowManager] Recording window exists:`,
      !!this.recordingWindow
    );
    console.log(
      `[WindowManager] Recording window destroyed:`,
      this.recordingWindow?.isDestroyed()
    );
    console.log(
      `[WindowManager] Recording window visible:`,
      this.recordingWindow?.isVisible()
    );

    // Validate window exists and is ready for animation
    if (!this.recordingWindow || this.recordingWindow.isDestroyed()) {
      console.warn(
        `[WindowManager] Cannot move recording window - window not available`
      );
      return;
    }
    ...
    if (!this.recordingWindow.isVisible()) {
      console.warn(
        `[WindowManager] Cannot move recording window - window not visible (user not authenticated or onboarding not complete)`
      );
      return;
    }
    ...
    console.log(
      `[WindowManager] Current window position: (${currentBounds.x}, ${currentBounds.y})`
    );
    console.log(
      `[WindowManager] Target position: (${position.x}, ${position.y})`
    );
    console.log(
      `[WindowManager] Animation duration: ${animationDuration || 200}ms`
    );
    ...
    console.log(
      `[WindowManager] *** STARTING INSTANT CROSS-DISPLAY POSITIONING ***`
    );
    console.log(
      `[WindowManager] Moving from (${currentBounds.x}, ${currentBounds.y}) to (${position.x}, ${position.y})`
    );
    ...
    try {
      ...
      console.log(
        `[WindowManager] Position after first setBounds: (${boundsAfterFirst.x}, ${boundsAfterFirst.y})`
      );
      ...
      if (
        boundsAfterFirst.x !== position.x ||
        boundsAfterFirst.y !== position.y
      ) {
        console.log(
          `[WindowManager] *** APPLYING SECOND SETBOUNDS (SCALING CORRECTION) ***`
        );
        console.log(
          `[WindowManager] Expected: (${position.x}, ${position.y}), Got: (${boundsAfterFirst.x}, ${boundsAfterFirst.y})`
        );
      ...
      console.log(
        `[WindowManager] Final position: (${finalBounds.x}, ${finalBounds.y})`
      );
      console.log(
        `[WindowManager] Position accuracy: ${finalBounds.x === position.x && finalBounds.y === position.y ? "EXACT" : "APPROXIMATE"}`
      );
    } catch (error) {
      console.error(
        `[WindowManager] Failed to position recording window:`,
        error
      );
    }
    ...
    if (this.informationWindow && !this.informationWindow.isDestroyed()) {
      ...
      if (informationPosition) {
        const windowState = this.informationWindow.isVisible()
          ? "visible"
          : "hidden";
        console.log(
          `[WindowManager] Moving information window (${windowState}) instantly to (${informationPosition.x}, ${informationPosition.y})`
        );
        try {
          ...
          console.log(
            `[WindowManager] Information window (${windowState}) positioned successfully on display ${displayId}`
          );
        } catch (error) {
          console.error(
            `[WindowManager] Failed to position information window:`,
            error
          );
        }
      }
  ...
  private handleRecordingWindowPositionUpdate(
    event: PositionUpdateEvent
  ): void {
    ...
    if (!this.recordingWindow || this.recordingWindow.isDestroyed()) {
      console.warn(
        `[WindowManager] Cannot update recording window position - window not available`
      );
      return;
    }
    ...
    if (!this.recordingWindow.isVisible()) {
      console.warn(
        `[WindowManager] Cannot update recording window position - window not visible`
      );
      return;
    }

    console.log(
      `[WindowManager] Updating recording window position on display ${displayId}:`,
      position
    );
    ...
    try {
      ...
      console.log(
        `[WindowManager] Successfully updated window position to (${position.x}, ${position.y})`
      );
    } catch (error) {
    ...
    if (this.informationWindow && !this.informationWindow.isDestroyed()) {
      ...
      if (informationPosition) {
        const windowState = this.informationWindow.isVisible()
          ...
          : "hidden";
        console.log(
          `[WindowManager] Updating information window (${windowState}) position on display ${displayId} to (${informationPosition.x}, ${informationPosition.y})`
        );
        try {
          this.informationWindow.setBounds({
            x: informationPosition.x,
          ...
          console.log(
            `[WindowManager] Information window (${windowState}) position updated successfully`
          );
        } catch (error) {
          console.error(
            `[WindowManager] Failed to update information window position:`,
            error
          );
        }
      }
