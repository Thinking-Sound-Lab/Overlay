...
export class DisplayManagerService extends EventEmitter {
  private static instance: DisplayManagerService;
  ...
  private recordingWindowDimensions: RecordingWindowDimensions = { width: 50, height: 10 };
  private isActive = false;

  private constructor(config: Partial<DisplayManagerConfig> = {}) {
    super();
    this.config = { ...DEFAULT_DISPLAY_CONFIG, ...config };
    this.mouseTrackingService = MouseTrackingService.getInstance(this.config);
    this.setupEventListeners();
  }

  public static getInstance(config?: Partial<DisplayManagerConfig>): DisplayManagerService {
    if (!DisplayManagerService.instance) {
      DisplayManagerService.instance = new DisplayManagerService(config);
    }
    return DisplayManagerService.instance;
  }

  private setupEventListeners(): void {
    // Listen for display changes from mouse tracking service
    this.mouseTrackingService.on('display-changed', (event: DisplayChangeEvent) => {
    ...
    screen.on('display-removed', (event, oldDisplay) => {
      ...
      if (this.currentRecordingDisplayId === oldDisplay.id) {
        ...
        console.log('[DisplayManagerService] Recording window display removed, moving to primary display');
    ...
    screen.on('display-metrics-changed', (event, display, changedMetrics) => {
      console.log('[DisplayManagerService] Display metrics changed:', display.id, changedMetrics);
      
      // If the recording window is on this display, recalculate position
      if (this.currentRecordingDisplayId === display.id && changedMetrics.includes('bounds' as any) || changedMetrics.includes('workArea' as any)) {
        console.log('[DisplayManagerService] Recording window display metrics changed, updating position');
  ...
  private handleDisplayChange(event: DisplayChangeEvent): void {
    if (!this.isActive) {
      console.log(`[DisplayManagerService] Display change ignored - service not active`);
      return;
    }

    console.log(`[DisplayManagerService] *** HANDLING DISPLAY CHANGE EVENT ***`);
    console.log(`[DisplayManagerService] Previous display:`, event.previousDisplay ? `ID ${event.previousDisplay.id}` : 'null');
    console.log(`[DisplayManagerService] Current display: ID ${event.currentDisplay.id}`);
    console.log(`[DisplayManagerService] Previous display bounds:`, event.previousDisplay?.bounds);
    console.log(`[DisplayManagerService] Current display bounds:`, event.currentDisplay.bounds);
    ...
    const displayIdsDifferent = event.previousDisplay?.id !== event.currentDisplay.id;
    
    console.log(`[DisplayManagerService] *** CONDITION CHECK ***`);
    console.log(`[DisplayManagerService] Has previous display: ${hasPreviousDisplay}`);
    console.log(`[DisplayManagerService] Previous display ID: ${event.previousDisplay?.id}`);
    console.log(`[DisplayManagerService] Current display ID: ${event.currentDisplay.id}`);
    console.log(`[DisplayManagerService] Display IDs different: ${displayIdsDifferent}`);
    console.log(`[DisplayManagerService] Should trigger move: ${hasPreviousDisplay && displayIdsDifferent}`);
    
    // Only move if display actually changed
    if (hasPreviousDisplay && displayIdsDifferent) {
      console.log(`[DisplayManagerService] *** TRIGGERING WINDOW MOVE *** from display ${event.previousDisplay!.id} to display ${event.currentDisplay.id}`);
      this.moveRecordingWindowToDisplay(event.currentDisplay.id);
    } else {
      console.warn(`[DisplayManagerService] *** NO WINDOW MOVE TRIGGERED *** - Conditions not met:`);
      console.warn(`[DisplayManagerService] - Has previous display: ${hasPreviousDisplay}`);
      console.warn(`[DisplayManagerService] - Display IDs different: ${displayIdsDifferent}`);
    }
  }
  ...
  public moveRecordingWindowToDisplay(displayId: number): void {
    ...
    console.log(`[DisplayManagerService] From display: ${previousDisplayId} → To display: ${displayId}`);
    ...
    if (position) {
      ...
      console.log(`[DisplayManagerService] *** EMITTING 'move-recording-window' EVENT ***`, moveEvent);
      
      this.emit('move-recording-window', moveEvent);
    } else {
      console.error(`[DisplayManagerService] Could not calculate position for display ${displayId}`);
    }
  }

  public calculateRecordingWindowPosition(displayId: number): WindowPosition | null {
    ...
    const { width: windowWidth, height: windowHeight } = this.recordingWindowDimensions;
    
    console.log(`[DisplayManagerService] *** CALCULATING POSITION WITH DISPLAY ORIGIN OFFSET ***`);
    console.log(`[DisplayManagerService] Target display ${displayId} bounds:`, bounds);
    console.log(`[DisplayManagerService] Target display ${displayId} workArea:`, workArea);
    console.log(`[DisplayManagerService] Window dimensions: ${windowWidth}x${windowHeight}`);
    ...
    console.log(`[DisplayManagerService] Recording window positioning calculation:`);
    console.log(`[DisplayManagerService] - Work area height: ${workArea.height}`);
    console.log(`[DisplayManagerService] - Recording window height: ${windowHeight}`);
    console.log(`[DisplayManagerService] - Taskbar margin: ${taskbarMargin}`);
    console.log(`[DisplayManagerService] - Calculated relative Y: ${relativeY}`);
    console.log(`[DisplayManagerService] *** COORDINATE CALCULATION ***`);
    console.log(`[DisplayManagerService] Relative position within display: (${relativeX}, ${relativeY})`);
    console.log(`[DisplayManagerService] Display bounds offset: (${bounds.x}, ${bounds.y})`);
    console.log(`[DisplayManagerService] Final global position: (${globalX}, ${globalY})`);
  ...
  public calculateInformationWindowPosition(displayId: number, recordingWindowHeight: number): WindowPosition | null {
    const display = this.mouseTrackingService.getDisplayById(displayId);
    
    if (!display) {
      console.error(`[DisplayManagerService] Display ${displayId} not found for information window`);
      return null;
    }
    ...
    const relativeY = workArea.height - recordingWindowHeight - informationWindowHeight - gap - taskbarMargin;
    
    // Convert to global coordinates by adding display bounds offset
    const globalX = bounds.x + relativeX;
    const globalY = bounds.y + relativeY;
    
    console.log(`[DisplayManagerService] Information window positioning calculation:`);
    console.log(`[DisplayManagerService] - Work area height: ${workArea.height}`);
    console.log(`[DisplayManagerService] - Recording window height: ${recordingWindowHeight}`);
    console.log(`[DisplayManagerService] - Information window height: ${informationWindowHeight}`);
    console.log(`[DisplayManagerService] - Gap: ${gap}`);
    console.log(`[DisplayManagerService] - Taskbar margin: ${taskbarMargin}`);
    console.log(`[DisplayManagerService] - Calculated relative Y: ${relativeY}`);
    console.log(`[DisplayManagerService] Information window position: relative (${relativeX}, ${relativeY}) → global (${globalX}, ${globalY})`);
  ...
  public setRecordingWindowDimensions(dimensions: RecordingWindowDimensions): void {
    ...
    if (this.currentRecordingDisplayId !== null) {
      const position = this.calculateRecordingWindowPosition(this.currentRecordingDisplayId);
      if (position) {
        this.emit('recording-window-position-update-needed', {
  ...
  public getCurrentRecordingDisplay(): DisplayInfo | null {
    ...
    return this.mouseTrackingService.getDisplayById(this.currentRecordingDisplayId);
  }
  ...
  public getDisplayNearestPoint(x: number, y: number): DisplayInfo | null {
    try {
      ...
      console.error('[DisplayManagerService] Error getting display nearest point:', error);
      return null;
    }
  }
  ...
  public getVirtualDesktopInfo(): VirtualDesktopInfo | null {
    ...
    console.log('[DisplayManagerService] Virtual desktop tracking not yet implemented');
    return null;
  }
