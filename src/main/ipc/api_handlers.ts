...
export class APIHandlers {
  ...
  private setupHandlers() {
    ...
    ipcMain.handle('auth:signInWithGoogle', this.handleSignInWithGoogle.bind(this));
    ...
    ipcMain.handle('auth:completeOnboarding', this.handleCompleteOnboarding.bind(this));
    console.log('APIHandlers: Registered auth:deleteAccount and auth:completeOnboarding handlers');
    ...
    ipcMain.handle('db:saveUserSettings', this.handleSaveUserSettings.bind(this));
    ...
    ipcMain.handle('analytics:trackUserSignUp', this.handleTrackUserSignUp.bind(this));
    ipcMain.handle('analytics:trackUserSignIn', this.handleTrackUserSignIn.bind(this));
    ipcMain.handle('analytics:trackUserSignOut', this.handleTrackUserSignOut.bind(this));
    ipcMain.handle('analytics:trackRecordingStarted', this.handleTrackRecordingStarted.bind(this));
    ipcMain.handle('analytics:trackRecordingStopped', this.handleTrackRecordingStopped.bind(this));
    ipcMain.handle('analytics:trackTranscriptionCompleted', this.handleTrackTranscriptionCompleted.bind(this));
    ipcMain.handle('analytics:trackAppLaunched', this.handleTrackAppLaunched.bind(this));
  ...
  // Authentication handlers
  private async handleSignIn(event: any, credentials: { email: string; password: string }): Promise<IPCResponse> {
    ...
    try {
      if (!credentials?.email || !credentials?.password) {
        return this.createResponse(null, new Error('Email and password are required'));
      }

      const result = await this.apiManager.supabase.signIn(credentials.email, credentials.password);
  ...
  private async handleSignUp(event: any, credentials: { email: string; password: string; name?: string }): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
    ...
    try {
      ...
      const result = await this.apiManager.supabase.signUp(credentials.email, credentials.password, credentials.name);
      return this.createResponse(result);
    } catch (error) {
      return this.createResponse(null, error);
    }
  ...
  // Database handlers
  private async handleSaveTranscript(event: any, transcript: any): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  private async handleGetTranscripts(event: any, limit?: number): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  private async handleSaveUserSettings(event: any, settings: any): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  // Analytics handlers
  private async handleTrackEvent(event: any, data: { event: string; properties?: Record<string, any> }): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  private async handleIdentifyUser(event: any, data: { userId: string; properties?: Record<string, any> }): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  private async handleTrackUserSignUp(event: any, method: 'email' | 'google' | 'github' = 'email'): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  private async handleTrackUserSignIn(event: any, method: 'email' | 'google' | 'github' = 'email'): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  private async handleTrackRecordingStopped(event: any, duration: number): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }
  ...
  private async handleTrackTranscriptionCompleted(event: any, data: { wordCount: number; wpm: number; wasTranslated: boolean }): Promise<IPCResponse> {
    if (!this.validateSender(event.sender)) {
      return this.createResponse(null, new Error('Unauthorized'));
    }

    try {
      this.apiManager.analytics.trackTranscriptionCompleted(data.wordCount, data.wpm, data.wasTranslated);
      return this.createResponse({ success: true });
    } catch (error) {
      return this.createResponse(null, error);
    }
